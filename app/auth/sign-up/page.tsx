...
const SignUp = () => {
    ...
    const handleSubmit = async (e: SubmitEvent<HTMLFormElement>) => {
        ...
        try {
            setLoading(true)
            setError(null)
            const {  error } = await supabase.auth.signUp({ email, password, options: { emailRedirectTo: 'http://localhost:3000' } })
        ...
        <div className="min-h-screen bg-zinc-950 flex items-center justify-center px-4">
            <Card className="w-full max-w-md p-8 bg-zinc-900 border-zinc-800">
                ...
                <p className="text-center text-zinc-400 mt-6">
                    Already have an account? <Link href={'/auth/login'} className="text-emerald-400 hover:text-emerald-300">Login</Link>
                </p>
            </Card>
        </div>
    )
